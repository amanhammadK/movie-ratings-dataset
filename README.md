...
A ready-to-query dataset server for **Movie ratings and metadata**. Records are shipped as JSON in `data/dataset.json` and exposed through a Model Context Protocol server with `query`, `get_record`, and `get_stats` tools.
...
Fields: title, year, genre, rating, votes
